...
elseif($option -eq "build")
{
    ...
    else
    {
        ...
        if($args[1] -eq "DEBUG" -or $args[1] -eq "ALL")
        {
			...
			copy-item "./build/Debug/dr_flac.lib" ($targetPath + "dr_flac.lib") -Force
			copy-item "./build/Debug/dr_mp3.lib" ($targetPath + "dr_mp3.lib") -Force
			copy-item "./build/Debug/dr_wav.lib" ($targetPath + "dr_wav.lib") -Force
			$built = $true
        }
		...
		if($args[1] -eq "RELEASE" -or $args[1] -eq "ALL")
        {
			...
			copy-item "./build/Release/dr_flac.lib" ($targetPath + "dr_flac.lib") -Force
			copy-item "./build/Release/dr_mp3.lib" ($targetPath + "dr_mp3.lib") -Force
			copy-item "./build/Release/dr_wav.lib" ($targetPath + "dr_wav.lib") -Force
			$built = $true
        }
